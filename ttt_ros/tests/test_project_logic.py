...
class FakeSimulator:
    ...
    def place_human_move(self, row, col):
        self.human_placements.append((row, col))
        return {"row": row, "col": col}

    def restore_scene(self):
        self.restore_calls += 1
        self.executed.clear()
        self.human_placements.clear()
        self.supply.reset()
...
class TestProjectApp(unittest.TestCase):
    ...
    def test_only_robot_turns_publish_and_execute(self):
        ...
        self.assertEqual(self.simulator.human_placements, [(0, 0)])
    ...
    def test_reset_restores_board_and_supply(self):
        ...
        self.assertEqual(self.simulator.human_placements, [])
    ...
    def test_restore_for_new_game_uses_animated_restore(self):
        self.app.handle_human_move(0, 0, 0)
        self.app.step(0)

        self.app.restore_for_new_game()

        self.assertEqual(self.simulator.restore_calls, 1)
        self.assertEqual(self.simulator.reset_calls, 0)
        self.assertEqual(self.app.controller.game.board, [[EMPTY] * 3 for _ in range(3)])
        self.assertIsNone(self.app.controller.robot_started_ms)
