...
def _ensure_ros_node(node_name="tic_tac_toe_project_node"):
    ...
    try:
        ...
        print(f"[project_real] rospy.init_node failed: {exc}")
...
# (e.g. gripper close/open, contact with a block).
...
class RealTicTacToeRobot:
    ...
    def _trajectory_durations(self, cart_waypoints):
        ...
        for i in range(len(ee_positions) - 1):
            ...
            speed_limited_duration = MIN_JERK_PEAK_SPEED_SCALE * cart_distance / self.max_eef_linear_speed
            segment_duration = max(nominal_duration, speed_limited_duration)
            if (
            ...
            segment_durations.append(segment_duration * self.motion_duration_scale)
    ...
    def _goto_pose(self, waypoint, duration, waypoint_name):
        pose = self._make_pose(waypoint)
        self._validate_pose_in_workspace(pose, f"{waypoint_name} target pose")
        self.fa.goto_pose(
            pose,
            duration=float(duration),
            use_impedance=self.use_impedance,
            cartesian_impedances=self.cartesian_impedances,
            **self._motion_kwargs(f"project_real:{waypoint_name}"),
        )
    ...
    def _goto_home(self):
        ...
        self.fa.goto_joints(
            ...
            **self._motion_kwargs("project_real:return_home"),
        )
        self._monitor_skill("return_home")

    def _run_cartesian_trajectory(self, cart_waypoints):
        ...
        print("[project_real] completed waypoint: return_home")
        self.current_q = Q_HOME.copy()

    def _execute_target_xyz(self, block_name, source_xyz, target_xyz, target_label):
        plan_t0 = time.time()
        print(
            f"[project_real] planning {block_name} for {target_label} "
            f"target=({target_xyz[0]:.3f}, {target_xyz[1]:.3f}, {target_xyz[2]:.3f})"
        )
        orientation_label, cart_waypoints, _joint_waypoints = self._select_motion_plan(source_xyz, target_xyz)
        plan_dt = time.time() - plan_t0
        print(
            f"[project_real] placing {block_name} at {target_label} "
            f"target=({target_xyz[0]:.3f}, {target_xyz[1]:.3f}, {target_xyz[2]:.3f}) "
            f"orientation={orientation_label} "
            f"planning_time={plan_dt:.2f}s"
        )
        self._validate_motion_plan(cart_waypoints, _joint_waypoints)
        self._run_cartesian_trajectory(cart_waypoints)
        return {
...
def parse_args(argv):
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Enable the FrankaArm collision-box/workspace-wall joint check (off by default).",
    )
    parser.add_argument("--disable-eef-speed-check", action="store_true")
